    ...
    if which == "reg":
        ...
        if not os.path.exists(temp_path+"app_launch_temp1.csv"):
            ...
            df_app_launch_train_temp["user_app_launch_register_type_rate"] = \
                (df_app_launch_train_temp.groupby(by=["register_type"])["register_type"].transform("count")).astype(np.uint32)
            df_app_launch_train_temp["user_app_launch_register_type_ratio"] = \
                (df_app_launch_train_temp["user_app_launch_register_type_rate"]/len(df_app_launch_train_temp)).astype(np.float32)
            df_app_launch_train_temp["user_app_launch_device_type_rate"] = \
                (df_app_launch_train_temp.groupby(by=["device_type"])["device_type"].transform("count")).astype(np.uint32)
            df_app_launch_train_temp["user_app_launch_device_type_ratio"] = \
                (df_app_launch_train_temp["user_app_launch_device_type_rate"]/len(df_app_launch_train_temp)).astype(np.float32)
            df_app_launch_train_temp["user_app_launch_register_device_ratio"] = \
                (df_app_launch_train_temp.groupby(by=["register_type","device_type"])["device_type"].transform("count") / df_app_launch_train_temp["user_app_launch_register_type_rate"]).astype(np.float32)
            df_app_launch_train_temp["user_app_launch_device_register_ratio"] = \
                (df_app_launch_train_temp.groupby(by=["device_type","register_type"])["register_type"].transform("count") / df_app_launch_train_temp["user_app_launch_device_type_rate"]).astype(np.float32)
        ...
        # gc.collect()
        def create_launch_feature(i,df_app_launch_train_temp_all=df_app_launch_train_temp_all,df_gp_temp=df_gp_temp_all["app_launch_day"]):
            ...
            if i==2:
                ...
                if not os.path.exists(temp_path+"app_launch_temp6.csv"):
                    ...
                    for rf in tqdm(range(0, 3, 1)):
                        ...
                        df_app_launch_train_temp_all[rate_name_forward] = (df_gp_temp.apply(
                            lambda x: count_occurence(x, (trainSpan[0], trainSpan[1] - rf+1)))).astype(np.uint8).values
                ...
                if not os.path.exists(temp_path + "app_launch_temp8.csv"):
                    ...
                    for rb in tqdm(range(0, 3, 1)):
                        ...
                        df_app_launch_train_temp_all[rate_name_backward] = (df_gp_temp.apply(
                            lambda x: count_occurence(x, (trainSpan[1] - rb, trainSpan[1] + 1)))).astype(np.uint8).values
                ...
                if not os.path.exists(temp_path + "app_launch_temp13.csv"):
                    ...
                    gc.collect()
            del df_app_launch_train_temp_all,df_gp_temp
            gc.collect()
            return i
        df_gp_temp1 = df_gp_temp_all["user_app_launch_register_diff"]
        df_gp_temp2 = df_gp_temp_all["app_launch_day"]
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_launch_feature,i,df_app_launch_train_temp_all,df_gp_temp1) for i in [2,3,4,5]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("rate ,var and gap thread ",future.result())
                except Exception as e:
                    print('app launch thread: rate ,var and gap feature creating process error. ' + str(e))
                else:
                    print('app launch thread: rate ,var and gap feature engineering process ok.')
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_launch_feature,i,df_app_launch_train_temp_all,df_gp_temp2) for i in [6,7,8,9]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("rate ,var and gap thread ",future.result())
                except Exception as e:
                    ...
                    print('app launch thread: rate ,var and gap feature engineering process ok.')
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_launch_feature,i,df_app_launch_train_temp_all,df_gp_temp2) for i in [10,11,12,13]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("rate ,var and gap thread ",future.result())
                except Exception as e:
        ...
        if not os.path.exists(temp_path + "app_launch_temp14.csv"):
            ...
            df_app_launch_train_temp_all[app_launch_feature_temp14].to_csv(app_launch_temp14_file, header=True, index=False)
            print(df_app_launch_train_temp_all.describe())
            del df_gp_temp,df_gp_temp_all,df_app_launch_train
            gc.collect()
        else:
            df_app_launch_train_temp_all = pd.read_csv(temp_path + "app_launch_temp14.csv", header=0,index_col=None)
            print(df_app_launch_train_temp_all.describe())
        ...
        print("finish getting basic app launch log features, get {} features ".format(len(list(df_app_launch_train_temp_all.columns)) - 1))
        # ds1.to_csv("kuaishou_stats2.csv", mode='a')
        file_name2 = "../input/fusai/basic_app_launch_feature_" + str(trainSpan[0]) + "_" + str(trainSpan[1]) + ".csv"
        ...
        if not os.path.exists(temp_path+"video_create_temp1.csv"):
            ...
            df_video_create_train_temp["user_video_create_register_type_rate"] = \
                (df_video_create_train_temp.groupby(by=["register_type"])["register_type"].transform("count")).astype(np.uint32)
            df_video_create_train_temp["user_video_create_register_type_ratio"] = \
                (df_video_create_train_temp["user_video_create_register_type_rate"]/len(df_video_create_train_temp)).astype(np.float32)
            df_video_create_train_temp["user_video_create_device_type_rate"] = \
                (df_video_create_train_temp.groupby(by=["device_type"])["device_type"].transform("count")).astype(np.uint32)
            df_video_create_train_temp["user_video_create_device_type_ratio"] = \
                (df_video_create_train_temp["user_video_create_device_type_rate"]/len(df_video_create_train_temp)).astype(np.float32)
            df_video_create_train_temp["user_video_create_register_device_ratio"] = \
                (df_video_create_train_temp.groupby(by=["register_type","device_type"])["device_type"].transform("count") / df_video_create_train_temp["user_video_create_register_type_rate"]).astype(np.float32)
            df_video_create_train_temp["user_video_create_device_register_ratio"] = \
                (df_video_create_train_temp.groupby(by=["device_type","register_type"])["register_type"].transform("count") / df_video_create_train_temp["user_video_create_device_type_rate"]).astype(np.float32)
        ...
        if not os.path.exists(temp_path+"video_create_temp2.csv"):
            ...
            df_video_create_train_temp_all[video_create_feature_temp2 + ["user_id"]].to_csv(video_create_feature_temp2_file,header=True,index=False)
            df_video_create_train_temp_all.drop(labels=video_create_feature_temp2,axis=1,inplace=True)
            gc.collect()
            del df_gp_temp
            gc.collect()
        def create_video_create_feature(i,df_video_create_train_temp_all,df_gp_temp=df_gp_temp_all["video_create_day"]):
            if i==3:
                if not os.path.exists(temp_path+"video_create_temp3.csv"):
                    ...
                    df_video_create_train_temp_all[video_create_feature_temp3 + ["user_id"]].to_csv(video_create_feature_temp3_file,header=True,index=False)
                    df_video_create_train_temp_all.drop(labels=video_create_feature_temp3,axis=1,inplace=True)
                    gc.collect()
            elif i in [4,5,6]:
                rf = (i-4)*2
                print("get the first {} days video create forward feature info ".format(trainSpan[1] - trainSpan[0] - rf))
                ...
                if not os.path.exists(video_create_feature_temprf_file):
                    ...
                    df_video_create_train_temp_all["user_video_create_register_time"]).astype(
                        np.float32)
                    df_video_create_train_temp_all[day_name_forward_resInv] = (df_video_create_train_temp_all[day_name_forward]/ \
                                                                       df_video_create_train_temp_all["user_video_create_register_time"]).astype(
                        np.float32)
                ...
                if not os.path.exists(temp_path+"video_create_temp12.csv"):
                    ...
                    df_video_create_train_temp_all[video_create_feature_temp12+["user_id"]].to_csv(video_create_feature_temp12_file, header=True,
                                                                               index=False)
                ...
                if not os.path.exists(temp_path+"video_create_temp13.csv"):
                    ...
                    df_video_create_train_temp_all[video_create_feature_temp13+["user_id"]].to_csv(video_create_feature_temp13_file, header=True,index=False)
                ...
                if not os.path.exists(temp_path+"video_create_temp14.csv"):
                    ...
                    df_video_create_train_temp_all[video_create_feature_temp14+["user_id"]].to_csv(video_create_feature_temp14_file, header=True,
                                                                               index=False)
                ...
                if not os.path.exists(temp_path+"video_create_temp15.csv"):
                    ...
                    df_video_create_train_temp_all[video_create_feature_temp15+["user_id"]].to_csv(video_create_feature_temp15_file, header=True,
                                                                               index=False)
                ...
                if not os.path.exists(temp_path+"video_create_temp17.csv"):
                    ...
                    gc.collect()
            del df_video_create_train_temp_all,df_gp_temp
            gc.collect()
            return i

        df_gp_temp1 = df_gp_temp_all["video_create_day"]
        df_gp_temp2 = df_gp_temp_all["user_video_create_register_diff"]
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_video_create_feature,i,df_video_create_train_temp_all,df_gp_temp1) for i in [3,4,5,6,7]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("rate ,var and gap thread ",future.result())
                except Exception as e:
                    print('video create thread:rate ,var and gap feature creating process error. ' + str(e))
                else:
                    print('video create thread: rate ,var and gap feature engineering process ok.')
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_video_create_feature,i,df_video_create_train_temp_all,df_gp_temp1) for i in [8,9,10,11,12,13]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("rate ,var and gap thread ",future.result())
                except Exception as e:
                    ...
                    print('video create thread: rate ,var and gap feature engineering process ok.')
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_video_create_feature,i,df_video_create_train_temp_all,df_gp_temp2) for i in [14,15,16,17]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("post-dup time series thread ",future.result())
                except Exception as e:
                    print('video create thread: post-dup time series feature creating process error. ' + str(e))
                else:
                    print('video create thread: post-dup time series feature engineering process ok.')
        del df_gp_temp1,df_gp_temp2
        gc.collect()
        ...
        if not os.path.exists(temp_path+"video_create_temp18.csv"):
            ...
            df_video_create_train_temp_all[video_create_feature_temp18].to_csv(video_create_feature_temp18_file, header=True,index=False)
        else:
            df_video_create_train_temp_all = pd.read_csv(temp_path+"video_create_temp18.csv",header=0,index_col=None)
            print(df_video_create_train_temp_all.describe())
        ...
        for i in range(1,18):
            ...
            df_video_create_train_temp_all = df_video_create_train_temp_all.merge(df_video_create_train_temp,how="left")
            del df_video_create_train_temp
            gc.collect()
        ...
        print("finish getting basic video creat log features, get {} features ".format(len(list(df_video_create_train_temp_all.columns)) - 1))
        # ds1.to_csv("kuaishou_stats2.csv", mode='a')
        file_name3 = "../input/fusai/basic_video_create_feature_" + str(trainSpan[0]) + "_" + str(trainSpan[1]) + ".csv"
        ...
        file_name4 = "../input/fusai/basic_user_activity_feature_" + str(trainSpan[0]) + "_" + str(trainSpan[1]) + ".csv"
        ...
        if not os.path.exists(temp_path+"user_activity_temp1.csv"):
            ...
            df_user_activity_train_temp["user_user_activity_register_type_rate"] = \
                (df_user_activity_train_temp.groupby(by=["register_type"])["register_type"].transform("count")).astype(np.uint32)
            df_user_activity_train_temp["user_user_activity_register_type_ratio"] = \
                (df_user_activity_train_temp["user_user_activity_register_type_rate"]/len(df_user_activity_train_temp)).astype(np.float32)
            df_user_activity_train_temp["user_user_activity_device_type_rate"] = \
                (df_user_activity_train_temp.groupby(by=["device_type"])["device_type"].transform("count")).astype(np.uint32)
            df_user_activity_train_temp["user_user_activity_device_type_ratio"] = \
                (df_user_activity_train_temp["user_user_activity_device_type_rate"]/len(df_user_activity_train_temp)).astype(np.float32)
            df_user_activity_train_temp["user_user_activity_register_device_ratio"] = \
                (df_user_activity_train_temp.groupby(by=["register_type","device_type"])["device_type"].transform("count") / df_user_activity_train_temp["user_user_activity_register_type_rate"]).astype(np.float32)
            df_user_activity_train_temp["user_user_activity_device_register_ratio"] = \
                (df_user_activity_train_temp.groupby(by=["device_type","register_type"])["register_type"].transform("count") / df_user_activity_train_temp["user_user_activity_device_type_rate"]).astype(np.float32)
            user_activity_feature_temp1 = ["user_id",
                "user_user_activity_register_type_ratio","user_user_activity_device_type_ratio",
                "user_user_activity_register_device_ratio","user_user_activity_device_register_ratio"]
        ...
        df_user_activity_train_temp_all = df_user_activity_train_temp_all.merge(df_user_activity_train[["user_id","user_user_activity_register_time"]].drop_duplicates(),how="left")
        ...
        if not os.path.exists(temp_path + "user_activity_temp2.csv"):
            ...
            df_user_activity_train_temp_all[user_activity_feature_temp2+["user_id"]].to_csv(user_activity_feature_temp2_file,header=True,index=False)
        ...
        print("begin to get the page and action info")
        df_gp_temp1 = df_gp_temp_all["page"]
        df_gp_temp2 = df_gp_temp_all["page", "action_type"]
        def create_page_feature(i,df_user_activity_train_temp_all,df_gp_temp1,df_gp_temp2):
            ...
            if not os.path.exists(user_activity_feature_temp_file ):
                ...
                df_user_activity_train_temp_all[page_ratio_name] = df_gp_temp1.apply(lambda x: list(x).count(i)/(len(x)+0.00000001)).astype(np.float32).values
                ...
                for j in tqdm(range(0, 6)):
                    ...
                    df_user_activity_train_temp_all[page_action_ratio_name] = df_gp_temp2.apply(lambda x: len(x.loc[(x["page"]==i)&(x["action_type"]==j)])/(len(x.loc[(x["page"]==i)])+0.00000001)).astype(np.float32).values
            ...
            del df_user_activity_train_temp_all,df_gp_temp1,df_gp_temp2
            gc.collect()
            return i
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_page_feature, i, df_user_activity_train_temp_all, df_gp_temp1,
                                     df_gp_temp2) for i in [0,1,2,3,4]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("page feature thread ",future.result())
                except Exception as e:
                    print('activity log  thread: page feature creating process error. ' + str(e))
                else:
                    print('activity log thread: page feature engineering process ok.')
        def create_action_feature(i,df_user_activity_train_temp_all,df_gp_temp1,df_gp_temp2):
            ...
            if not os.path.exists(user_activity_feature_temp_file):
                ...
                df_user_activity_train_temp_all[action_type_ratio_name] = df_gp_temp1.apply(lambda x: list(x).count(i)/(len(x)+0.00000001)).astype(np.float32).values
            ...
            return i
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_action_feature, i, df_user_activity_train_temp_all, df_gp_temp1,
                                     df_gp_temp2) for i in [0,1,2,3,4,5]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("action feature thread ",future.result())
                except Exception as e:
                    print('activity log thread: action feature creating process error. ' + str(e))
                else:
                    print('activity log thread: action feature engineering process ok.')
        del df_gp_temp1, df_gp_temp2
        gc.collect()
        df_user_activity_train.drop(labels=["page", "action_type"], axis=1, inplace=True)
        gc.collect()
        print("begin to get time series and var features ")
        def create_ts_daygap_feature(i, df_user_activity_train_temp_all, df_gp_temp):
            if i==1:
                if not os.path.exists(temp_path+"user_activity_temp14.csv"):
                    ...
                    df_user_activity_train_temp_all[user_activity_feature_temp14+["user_id"]].to_csv(user_activity_feature_temp14_file,header=True,index=False)
                ...
                if not os.path.exists(temp_path + "user_activity_temp15.csv"):
                    ...
                    gc.collect()
            del df_user_activity_train_temp_all,df_gp_temp
            gc.collect()
            return i

        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            tp1 = executor.submit(create_ts_daygap_feature, 1, df_user_activity_train_temp_all, df_gp_temp_all["user_user_activity_register_diff"])
            tp2 = executor.submit(create_ts_daygap_feature, 2, df_user_activity_train_temp_all, df_gp_temp_all["user_activity_day"])
            tps = [tp1,tp2]
            for tp in concurrent.futures.as_completed(tps):
                try:
                    res = tp.result()
                    print("pre-dup time series and var thread ",res)
                except Exception as e:
                    print('activity log thread: feature creating process error. ' + str(e))
                else:
                    print('activity log thread: feature engineering process ok.')
        gc.collect()
        print("begin to get the farward and backward rate features")
        def create_activity_rf_feature(i,df_user_activity_train_temp_all,df_gp_temp=df_gp_temp_all["user_activity_day"]):
            rf = i
            ...
            if not os.path.exists(user_activity_feature_temprf_file):
                ...
                df_user_activity_train_temp_all[frequency_name_forward] = (
                    df_user_activity_train_temp_all[rate_name_forward] / (df_user_activity_train_temp_all[day_name_forward]+0.00000001)).astype(
                    np.float32)

                df_user_activity_train_temp_all[rate_name_forward_resInv] = (df_user_activity_train_temp_all[rate_name_forward]/ \
                                                                   df_user_activity_train_temp_all[
                                                                       "user_user_activity_register_time"]).astype(
                    np.float32)

                df_user_activity_train_temp_all[day_name_forward_resInv] = (df_user_activity_train_temp_all[day_name_forward]/ \
                                                                   df_user_activity_train_temp_all[
                                                                       "user_user_activity_register_time"]).astype(
                    np.float32)

                user_activity_feature_temprf = [rate_name_forward,day_name_forward,frequency_name_forward,day_name_forward_resInv,rate_name_forward_resInv]
        ...
        def create_activity_rb_feature(i,df_user_activity_train_temp_all,df_gp_temp=df_gp_temp_all["user_activity_day"]):
            rb = i
            ...
            if not os.path.exists(user_activity_feature_temprb_file):
                ...
                df_user_activity_train_temp_all[rate_name_backward] = (df_gp_temp.apply(
                    lambda x: count_occurence(x, (trainSpan[1] - rb, trainSpan[1] + 1)))).astype(np.uint16).values
                df_user_activity_train_temp_all[rate_name_backward_resInv] = (df_user_activity_train_temp_all[rate_name_backward]/ \
                                                                     df_user_activity_train_temp_all[
                                                                         "user_user_activity_register_time"]).astype(
                    np.float32)
                df_user_activity_train_temp_all[day_name_backward] = (df_gp_temp.apply(
                    lambda x: count_occurence(set(x), (trainSpan[1] - rb, trainSpan[1] + 1)))).astype(np.uint8).values
                df_user_activity_train_temp_all[day_name_backward_resInv] = (df_user_activity_train_temp_all[day_name_backward] / \
                                                                    df_user_activity_train_temp_all[
                                                                        "user_user_activity_register_time"]).astype(
                    np.float32)
                if rb != 0:
        ...
        df_gp_temp = df_gp_temp_all["user_activity_day"]
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_activity_rf_feature, i, df_user_activity_train_temp_all,
                                     df_gp_temp) for i in [0,1,2,3,4,5]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("forward rate thread ",future.result())
                except Exception as e:
                    print('activity log thread: forward rate feature creating process error. ' + str(e))
                else:
                    print('activity log thread:forward rate  feature engineering process ok.')
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_activity_rb_feature, i, df_user_activity_train_temp_all,
                                     df_gp_temp) for i in [0,1,2,3,4,5]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("backward rate thread ",future)
                except Exception as e:
                    print('activity log thread: backward rate  feature creating process error. ' + str(e))
                else:
                    print('activity log thread: backward rate feature engineering process ok.')
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_activity_rb_feature, i, df_user_activity_train_temp_all,
                                     df_gp_temp) for i in [6,7,8,9,10,11]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("backward rate thread ",future.result())
                except Exception as e:
                    print('activity log thread: backward rate  feature creating process error. ' + str(e))
                else:
                    print('activity log thread: backward rate feature engineering process ok.')
        # df_user_activity_train.drop_duplicates(inplace=True)
        def create_gap_var_feature(i,df_user_activity_train_temp_all,df_gp_temp=df_gp_temp_all["user_activity_day"]):
            if i ==1:
            ...
            return i
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_gap_var_feature, i, df_user_activity_train_temp_all,
                                df_gp_temp) for i in [1, 2]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("gap and var thread ",future.result())
                except Exception as e:
                    print('activity log thread: gap and var  feature creating process error. ' + str(e))
                else:
                    print('activity log thread: gap and var feature engineering process ok.')
        del df_gp_temp
        gc.collect()
        def create_ts_feature(i,df_user_activity_train_temp_all,df_gp_temp=df_gp_temp_all["user_user_activity_register_diff"]):
            if i ==1:
                # df_gp_temp = df_gp_temp_all["user_user_activity_register_diff"]
        ...
        df_gp_temp = df_gp_temp_all["user_user_activity_register_diff"]
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(create_ts_feature, i, df_user_activity_train_temp_all,df_gp_temp) for i in [1,2,3]]
            for future in concurrent.futures.as_completed(futures):
                try:
                    print("post time series thread ",future.result())
                except Exception as e:
                    print('activity log thread: post time series feature creating process error. ' + str(e))
                else:
                    print('activity log thread: post time series feature engineering process ok.')
        del df_gp_temp
        gc.collect()
        if not os.path.exists(temp_path + "user_activity_temp39.csv"):
            ...
            df_user_activity_train_temp_all[user_activity_feature_temp39].to_csv(
            ...
            df_user_activity_train_temp_all = pd.read_csv(temp_path+"user_activity_temp39.csv",header=0,index_col=None)
            print(df_user_activity_train_temp_all.describe())
        ...
        for i in range(1,39):
            ...
            df_user_activity_train_temp_all = df_user_activity_train_temp_all.merge(df_user_activity_train_temp,how="left")
            del df_user_activity_train_temp
            gc.collect()
        ...
        df_user_activity_train_temp_all["video_author_ratio"] = (df_user_activity_train_temp_all["user_video_rate"]/df_user_activity_train_temp_all["user_author_rate"]).astype(np.float32)
        df_user_activity_train_temp_all["user_video_ratio"] = (df_user_activity_train_temp_all["user_video_rate"]/df_user_activity_train_temp_all["user_user_activity_day_f0"]).astype(np.float32)
        df_user_activity_train_temp_all["user_author_ratio"] = (df_user_activity_train_temp_all["user_author_rate"]/df_user_activity_train_temp_all["user_user_activity_day_f0"]).astype(np.float32)
        ...
        print(
            "finish getting basic user activity log features, get {} features ".format(len(df_user_activity_train_temp_all.columns) - 1))
        file_name4 = "../input/fusai/basic_user_activity_feature_" + str(trainSpan[0]) + "_" + str(trainSpan[1]) + ".csv"
...
def processing(trainSpan=(8, 23), label=True):
    ...
    if label:
        ...
        df_app_launch = pd.read_csv("../input/fusai/app_launch_log.txt", header=0, index_col=None,
                                    dtype=dtype_app_launch)
        active_user = (df_app_launch.loc[(df_app_launch["app_launch_day"] > trainSpan[1]) & (
    ...
    df_launch_activity_register_create["user_app_launch_video_ratio"] = (df_launch_activity_register_create["user_video_create_day_f0"]/(df_launch_activity_register_create["user_app_launch_rate_f0"]+0.00000001)).astype(np.float32)
    df_launch_activity_register_create["user_app_launch_activity_ratio"] = (df_launch_activity_register_create["user_user_activity_day_f0"]/(df_launch_activity_register_create["user_app_launch_rate_f0"]+0.00000001)).astype(np.float32)
    ...
    file_name5 = "../input/final_feature_" + str(trainSpan[0]) + "_" + str(trainSpan[1]) + ".csv"
    df_launch_activity_register_create.to_csv(file_name5, header=True, index=False)
    del df_user_register_train,df_user_register,df_user_activity_train,df_launch_register,df_app_launch_train,df_launch_register_create,df_video_create_train,df_ls
    gc.collect()

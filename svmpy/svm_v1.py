...
def predict(clf2, test_set):
    ...
    print(active_users)
    str_time = str(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M"))
    submission_file = "result/submission_svc_" + str_time + ".csv"
...
def run():
    ...
    clf1 = GridSearchCV(SVC(**initial_params),
                      param_grid={
    ...
    with open("kuaishou_stats.csv", 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["feature importance of svm for kuaishou-crt ", str_time])
        writer.writerow(["best score",bs,"best params"])
        for key, value in bp.items():
            writer.writerow([key, value])

    model_name = "svm_" + str_time + ".pkl"
    ...
    def tune_parameter(X, y, clf, params):
        ...
        gs = BayesSearchCV(
            ...
            verbose=2, n_jobs=-1, cv=4, refit=True, random_state=1234
        )
